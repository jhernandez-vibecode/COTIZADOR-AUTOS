...
**Resuelto y EN PRODUCCIÓN el 19 sep 2026 (merge `6b40576`, tag `pre-asistencia-por-plan-19sep`):** la guía muestra la tabla según plan × (M en `cb` → Extendido; solo G → G; sin `cb` → G como piso + nota); >20 años sin cifras. **Decisión de JC: opción B** — Básico con M muestra el piso de G + nota cualitativa, sin las cifras ⚠, hasta que el INS confirme la tabla que falta. Test: `tests/test-asistencia-por-plan.js`. Script: `docs/superpowers/specs/2026-09-19-asistencia-por-plan-mock.py`.
